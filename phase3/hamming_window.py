...
# Task 2.1: Create FIR Filter Bank
def create_fir_filter_bank(band_ranges, samplerate, num_taps=101):
    """
    Creates FIR filters for each band in the filter bank.
    """
    filters = []
    ...
    for low, high in band_ranges:
        ...
        fir_coeff = firwin(num_taps, [normalized_low, normalized_high], pass_zero=False)
...
def task_8_extract_envelopes(rectified_signals, samplerate, num_taps=101, cutoff=400):
    ...
    lowpass_filter = firwin(num_taps, normalized_cutoff, pass_zero=True)
    envelopes = [lfilter(lowpass_filter, 1.0, signal) for signal in rectified_signals]
    return envelopes
...
def main():
    ...
    play_and_save_output(output_signal, samplerate, output_filename=f"{file_name}.wav")
